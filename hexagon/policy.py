...
stays with hexagon proper; Ansible only enforces config *inside* qubes. The
two plain qrexec grants hexagon's own verbs need from that MgmtVM ride along
on the same tags: ``qubes.VMShell`` (``hexagon reboot`` powers a netvm off from
inside when clients are attached) and ``qubes.StartApp`` (``reboot -t``).
